...
const parse_id_state_pair_t parse_id_state_pair[] = {
	...
	{LINSEN_SERVO_0_POS_WRITE_STRING,  STATE_RX_SERVO_0_POS},
	{"quit",                           STATE_EXIT},
	{"exit",                           STATE_EXIT}
};
...
int linSen_socket_server_process(void) {
	...
	switch (state) {
		...
		case STATE_RX_SERVO_0_POS: {
			/* set servo 0 position data */
			int _val = atoi(&parse_buffer[strlen(LINSEN_SERVO_0_POS_WRITE_STRING)]);
			verbose_printf("\treceived(%s): set servo position to %d!", parse_buffer, _val);
			linSen_servo_set_pos(0, _val);
