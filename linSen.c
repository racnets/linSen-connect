...
linSen_interface_t linSen_get_interface = interface_NONE;
...
int linSen_init(const char* dev, linSen_interface_t interface) {
	if (interface == interface_I2C) {
		linSen_get_interface = interface;
...
int linSen_close(void) {
	if (linSen_get_interface == interface_I2C) {
		i2c_close();
	} else if (linSen_get_interface == interface_SOCKET) {
...
int linSen_process(void) {
	...
	if (linSen_get_interface == interface_SOCKET) {
		result = linSen_socket_client_process();
...
int linSen_get_pxclk(void) {
	...
	switch (linSen_get_interface) {
		...
		case interface_SOCKET: {
			result = linSen_socket_client_read(LINSEN_PXI_CLK_READ_STRING, &value);
			if (result < 0) return result;
			break;
		}
		default:;
